...
/**
 * Authentication Plugin: mysignup Authentication
 *
...
/**
 * mysignup authentication plugin.
 */
class auth_plugin_mysignup extends auth_plugin_base {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->authtype = 'mysignup';
        $this->config = get_config('auth_mysignup');
    }
     ...
     */
    public function auth_plugin_mysignup() {
    ...
    public function user_signup_with_confirmation($user, $notify=true, $confirmationurl = null) {
        ...
        if (!empty($SESSION->wantsurl)) {
            set_user_preference('auth_mysignup_wantsurl', $SESSION->wantsurl, $user);
        }
        ...
        if (! send_confirmation_email($user, $confirmationurl)) {
            throw new \moodle_exception('auth_mysignupnomysignup', 'auth_mysignup');
        }

        if ($notify) {
            global $CFG, $PAGE, $OUTPUT;
            $mysignupconfirm = get_string('mysignupconfirm');
            $PAGE->navbar->add($mysignupconfirm);
            $PAGE->set_title($mysignupconfirm);
            $PAGE->set_heading($PAGE->course->fullname);
            echo $OUTPUT->header();
            notice(get_string('mysignupconfirmsent', 'theme_nextgenis', $user->mysignup), "$CFG->wwwroot/index.php");
        } else {
            return true;
        }
    }
    ...
    function user_confirm($username, $confirmsecret) {
        ...
        if (!empty($user)) {
            if ($user->auth != $this->authtype) {
                ...
                if ($wantsurl = get_user_preferences('auth_mysignup_wantsurl', false, $user)) {
                    // Ensure user gets returned to page they were trying to access before signing up.
                    $SESSION->wantsurl = $wantsurl;
                    unset_user_preference('auth_mysignup_wantsurl', $user);
                }
