...
/**
 * Auth mysignup external functions tests.
 *
...
namespace auth_mysignup\external;

use auth_mysignup_external;
...
/**
 * External auth mysignup API tests.
 *
 * @package     auth_mysignup
 * @copyright   2016 Juan Leyva
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class external_test extends externallib_advanced_testcase {
    ...
    public function test_get_signup_settings() {
        ...
        $result = auth_mysignup_external::get_signup_settings();
        $result = \external_api::clean_returnvalue(auth_mysignup_external::get_signup_settings_returns(), $result);
    ...
    public function test_signup_user() {
        ...
        // Create new user.
        $result = auth_mysignup_external::signup_user($username, $password, $firstname, $lastname, $mysignup, $city,  $country,
                                                    '', '', $customprofilefields);
        $result = \external_api::clean_returnvalue(auth_mysignup_external::signup_user_returns(), $result);
        $this->assertTrue($result['success']);
        $this->assertEmpty($result['warnings']);
        $user = $DB->get_record('user', array('username' => $username));
        $this->assertEquals($firstname, $user->firstname);
        $this->assertEquals($lastname, $user->lastname);
        $this->assertEquals($mysignup, $user->mysignup);
        ...
        $this->assertEquals('mysignup', $user->auth);
        ...
        // Try to create a user with the same username, mysignup and password. We ommit also the profile fields.
        $password = 'abc';
        $result = auth_mysignup_external::signup_user($username, $password, $firstname, $lastname, $mysignup, $city,  $country,
                                                    '', '', $customprofilefields);
        $result = \external_api::clean_returnvalue(auth_mysignup_external::signup_user_returns(), $result);
        ...
        $expectederrors = array('username', 'mysignup', 'password');
        $finalerrors = [];
